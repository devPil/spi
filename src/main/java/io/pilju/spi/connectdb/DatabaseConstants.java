package io.pilju.spi.connectdb;

public class DatabaseConstants {
//    jdbc:mariadb://localhost:3306
//    public static final String CONFIG_KEY_JDBC_DRIVER = "org.h2.Driver";
    public static final String CONFIG_KEY_JDBC_DRIVER = "org.mariadb.jdbc";
    public static final String CONFIG_KEY_JDBC_URL = "jdbc:mariadb://localhost:3306/TEST";
    public static final String CONFIG_KEY_DB_USERNAME = "root";
    public static final String CONFIG_KEY_DB_PASSWORD = "root";
    public static final String CONFIG_KEY_VALIDATION_QUERY = "validationQuery";
}
